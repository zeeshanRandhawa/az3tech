...
const queryDRoutesFilter = async (filterData) => {
  try {
    let distinctDRouteSearchQuery = `SELECT DISTINCT(droute_id) FROM droutenodes WHERE node_id=${filterData.nodeId} AND ((arrival_time>=\'${filterData.startTimeWindow}:00\' AND arrival_time<=\'${filterData.endTimeWindow}:00\') OR (departure_time>=\'${filterData.startTimeWindow}:00\' AND departure_time<=\'${filterData.endTimeWindow}:00\')) AND status<>\'ORIGIN\'`;
    let drouteIdBetweenTime = (await pool.query(distinctDRouteSearchQuery)).rows;

    if (drouteIdBetweenTime.length > 0) {

      let drouteGetQuery = `SELECT * FROM droutes WHERE droute_id IN (${drouteIdBetweenTime.map(id => id.droute_id).join(',')})`;
      let droutesBetweenTime = (await pool.query(drouteGetQuery)).rows;

      droutesBetweenTime = await Promise.all(droutesBetweenTime.map(async (drouteData) => {
        ...
        let rNodesRes = (await pool.query(`SELECT droute_id, node_id, permutation_id, arrival_time, departure_time, rank, cum_distance, cum_time, capacity_used, status from droutenodes WHERE droute_id = ${drouteData.droute_id} AND outb_driver_id = ${drouteData.driver_id}`)).rows;
